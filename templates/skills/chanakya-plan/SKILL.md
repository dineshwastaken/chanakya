---
name: chanakya-plan
description: Chanakya planner — scope work, write numbered plan to local state. Use before implementation.
...
- Read `.chanakya/manifest.yaml` and any local state in `.chanakya/state/` (`active-context.md`, `progress.md`) if available.
- Use the distilled knowledge bundle instead of scanning raw source folders.

When planning:

- Trace the real end-to-end flow first (definition -> service -> UI as relevant).
...
- End by writing the plan into `.chanakya/state/active-context.md` (creating the directory if needed) so the implementer can execute it in a fresh chat.
